...
from .model_rsfa import Obj_MLPEnc, Obj_MLPDec, Edge_MLPEnc, Edge_MLPDec, CVAE
from .utils_relation import layer_init, get_box_info, get_box_pair_info
from maskrcnn_benchmark.data import get_dataset_statistics
from .loss import CVAE_LOSS, ReweightLoss


@registry.ROI_RELATION_PREDICTOR.register("RSFAPredictor")
class RSFAPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(RSFAPredictor, self).__init__()
        self.cfg = config
        ...
        self.obj_encoder = Obj_MLPEnc(config, obj_classes, in_channels)
        self.obj_decoder = Obj_MLPDec(config, obj_classes)
        self.edge_encoder = Edge_MLPEnc(config, rel_classes)
        self.edge_decoder = Edge_MLPDec(config)
        
        self.add_cvae = config.MODEL.ROI_BOX_HEAD.ADD_CVAE
        if self.add_cvae:
            cvae_hid_dim = self.hidden_dim // 2
            cvae_lat_dim = self.hidden_dim // 4
            self.cvae = CVAE(in_dim=self.hidden_dim, hid_dim=cvae_hid_dim, lat_dim=cvae_lat_dim, cls_dim=self.num_obj_cls)
            self.sample_only_fake = config.MODEL.ROI_BOX_HEAD.SAMPLE_ONLY_FAKE
            
        self.rel_use_reweight_loss = config.MODEL.ROI_RELATION_HEAD.USE_REWEIGHT_LOSS
        if self.rel_use_reweight_loss:
            self.rel_reweight_loss  =ReweightLoss(config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES,statistics['rel_per_cls'])
        
        if config.GLOBAL_SETTING.DATASET_CHOICE == 'GQA_200':
            rel_samples = config.MODEL.ROI_RELATION_HEAD.GQA_REL_SAMPLES
            self.z_rel_freq=sorted(rel_samples)[100]/torch.tensor(rel_samples,dtype=torch.float32)
        elif config.GLOBAL_SETTING.DATASET_CHOICE == 'VG':
            rel_samples=config.MODEL.ROI_RELATION_HEAD.REL_SAMPLES
            self.z_rel_freq=sorted(rel_samples)[5]/torch.tensor(rel_samples,dtype=torch.float32)
        
        # self.head_idxs=[31,20,48,30,22,29,50,8,21,1,40,49,43,38,23,41]
        # self.mid_idxs=[6,7,11,46,33,9,16,25,47,19,35,24,14,5,13,44,10]
        # self.tail_idxs=[12,4,36,26,32,42,28,45,2,3,17,18,34,27,37,39,15]

        # self.head_nums = [1829, 2273, 3167, 3757, 4260, 4810, 5086, 6712, 10011, 10764, 11059, 15300, 21748, 31347, 47326,
        #              67144, 109355]
        # self.mid_nums = torch.tensor([394, 397, 429, 460, 504, 551, 565, 601, 644, 663, 676, 752, 793, 809, 1225, 1413, 1603])
        # self.tail_nums = [4, 44, 71, 114, 151, 157, 163, 171, 208, 234, 270, 299, 333, 352, 364, 379]

        # self.mid_freq=self.mid_nums.min()/self.mid_nums
        
    def forward(self,proposals,rel_pair_idxs,rel_labels,rel_binarys,roi_features,union_features,logger=None,iterations=None):
        num_objs = [len(p) for p in proposals]
        num_rels = [r.size(0) for r in rel_pair_idxs]
        sem_feats, app_feats, obj_labels, pos_embed = self.obj_encoder(roi_features, proposals, logger)
        obj_dists, obj_preds, obj_embed_out = self.obj_decoder(proposals, sem_feats, obj_labels)
        
        obj_feats = cat((app_feats, obj_embed_out), dim=-1)
        
        # add cvae
        add_losses = {}
        if self.add_cvae and self.cvae.enc_fc_1.weight.requires_grad and self.training:
            one_hot_preds = torch.zeros((obj_preds.size(0), self.num_obj_cls), device=obj_preds.device)
            one_hot_preds[torch.arange(obj_preds.size(0)), obj_labels.detach()] = 1
            recon_feats, mu, logvar = self.cvae(app_feats.detach(), one_hot_preds)
            cvae_loss = CVAE_LOSS(recon_feats, app_feats.detach(), mu, logvar)
            add_losses.update(dict(cvae_loss=cvae_loss[0] + cvae_loss[1]))

        # add cvae composition samples
        K = 5   # TODO: ablation study
        Num = 1
        sample_feats = None
        if self.add_cvae and self.cfg.MODEL.ROI_HEADS.TRAIN_2STAGE and self.training and not \
                self.cfg.MODEL.ROI_RELATION_HEAD.RESAMPLING and not self.cfg.MODEL.ROI_BOX_HEAD.RESAMPLING and not \
                self.cfg.MODEL.ROI_RELATION_HEAD.USE_REWEIGHT_LOSS and not self.cfg.MODEL.ROI_BOX_HEAD.USE_REWEIGHT_LOSS:
            N_obj, N_k = app_feats.shape[0], 128
            lat = torch.randn((N_obj, K, N_k), device=obj_feats.device).view(-1, N_k)
            cond = torch.zeros((N_obj*K, self.num_obj_cls), device=app_feats.device)
            K_labels = obj_labels.repeat(K).view(K, N_obj).T.flatten()
            cond[torch.arange(N_obj*K), K_labels] = 1
            sample_feats = self.cvae.decode(lat, cond).view(N_obj, K, -1).detach()   # (n,K,512)

        rel_feats, union_embss, pair_preds, z_rel_feats, pair_labels = self.edge_encoder(proposals, rel_pair_idxs, union_features, obj_feats, obj_preds, obj_embed_out, sample_feats, obj_labels)
        rel_dists = self.edge_decoder(rel_feats, union_embss, pair_preds)
        
        # add sample_feats dists
        z_rel_dists = None
        if z_rel_feats is not None and self.training:
            rel_label = cat(rel_labels, dim=0)
            if self.sample_only_fake:
                z_rel_feats = z_rel_feats[:, -1]
                z_rel_dists = self.edge_decoder(z_rel_feats, union_embss, pair_preds)
                self.z_rel_freq = self.z_rel_freq.to(z_rel_dists.device)
                
                prop = torch.rand(rel_label.shape[0]).to(z_rel_dists.device)
                pred_rel = rel_dists[:,1:].max(1)[1] + 1
                if iterations < 60000:
                    used = (prop < self.z_rel_freq[rel_label.long()])
                else:
                    used=(prop<self.z_rel_freq[rel_label.long()]) & (pred_rel==rel_label)
                
                rel_label = rel_label.repeat(Num)
                used = used.repeat(Num)
                z_rel_loss = nn.CrossEntropyLoss(reduction='none')(z_rel_dists, rel_label)
                
                z_rel_loss = (z_rel_loss * used).sum() / (used.sum() + 1e-6)
                add_losses.update(dict(z_rel_loss=z_rel_loss))
        
        # add rel re-balancing loss
        if self.rel_use_reweight_loss and self.training:
            rel_label = cat(rel_labels, dim=0)
            assert rel_dists.shape[0] == rel_label.shape[0]
            rel_reweight_loss = self.rel_reweight_loss(rel_dists, rel_label)
            add_losses.update(dict(rel_reweight_loss=rel_reweight_loss))
