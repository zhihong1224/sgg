...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.union_feature_extractor = make_roi_relation_feature_extractor(cfg, in_channels, cfg.MODEL.ROI_RELATION_HEAD.ROI_ALIGN)
        if cfg.MODEL.ATTRIBUTE_ON:
            ...
            self.box_feature_extractor = make_roi_box_feature_extractor(cfg, in_channels, cfg.MODEL.ROI_BOX_HEAD.ROI_ALIGN)
    ...
    def forward(self, features, proposals, targets=None, logger=None, iterations=None):
        """
        Arguments:
        ...
        refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger, iterations)
        ...
        # TODO
        if self.cfg.MODEL.ROI_HEADS.TRAIN_2STAGE:
            if self.cfg.MODEL.ROI_BOX_HEAD.TRAIN_OBJ_CLS and self.cfg.MODEL.ROI_RELATION_HEAD.TRAIN_REL_CLS:
                output_losses=dict(loss_rel=loss_relation)    # data of dataset use loss_rel, data of composition use z_loss_rel, data of reweight use loss_reweight
            elif self.cfg.MODEL.ROI_BOX_HEAD.TRAIN_OBJ_CLS:
                output_losses=dict(loss_refine_obj=loss_refine)
            elif self.cfg.MODEL.ROI_RELATION_HEAD.TRAIN_REL_CLS:
            ...
            elif self.cfg.MODEL.ROI_BOX_HEAD.USE_REWEIGHT_LOSS:
                output_losses=dict(loss_rel=loss_relation)
            elif self.cfg.MODEL.ROI_RELATION_HEAD.USE_REWEIGHT_LOSS:
                output_losses=dict(loss_refine_obj=loss_refine)
            else:
        ...
        return roi_features, proposals, output_losses   # ,rel_feats,rel_label
