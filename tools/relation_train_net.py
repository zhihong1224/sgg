...
def train(cfg, local_rank, distributed, logger):
    ...
    model, optimizer = amp.initialize(model, optimizer, opt_level=amp_opt_level)  # TODO
    ...
    if checkpointer.has_checkpoint():
        ...
        if cfg.MODEL.ROI_HEADS.TRAIN_2STAGE: # TODO: if train_2stage, fix parameters
            checkpointer.load(cfg.MODEL.PRETRAINED_DETECTOR_CKPT, with_optim=False)
            for name, param in model.named_parameters():
                if 'edge_decoder' not in name:
                    param.requires_grad = False
            print('train the second stage...')
        else:
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        loss_dict = model(images, targets, iterations=iteration)
        
        # TODO: compute FLOPs and Params
        # import pdb;pdb.set_trace()
        # macs, FLOPs = profile(model, inputs=(images,targets,None,iteration))
        # macs, FLOPs = clever_format([macs,FLOPs], "%.2f")
        # print('FLOPs: ',FLOPs)
